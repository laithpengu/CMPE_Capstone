...
void reset();// resets the transciever
byte read_short(byte address);// performs a short read
byte read_long(word address);//performs a long read
void write_short(byte address, byte data);// performs a short write
void write_long(word address, byte data);// performs a long write
word get_pan();//gets the pan id
void address16_write(word address);// writes the short address of the RF
word address16_read();//gets the address of the rf
void set_interupts();//sets the intcon register in the rf
void set_channel();// sets the channel of the rf
void module_init();// initializes the module by writing the respective registers
void set_promiscuous(boolean enabled);//sets promiscuus bit
rx_info_t * get_rxinfo();//gets the rx info 
tx_info_t * get_txinfo();//gets the tx_info
uint8_t * get_rxbuf();// gets the rx buf
int rx_datalength(); // returns data len
